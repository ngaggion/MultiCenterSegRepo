...
if __name__ == "__main__":
    ...
    if config['original']:
        ...
        if config['organs'] == 'LHC':
            images = [image for image in images if "JP" in image]
    else:
        images1 = open("train_images_lungs.txt",'r').read().splitlines()
        images2 = open("train_images_heart.txt",'r').read().splitlines()
        images = images1 + images2
    
    if config['organs'] == 'LH' and config['pad']:
        images2 = open("train_images_heart.txt",'r').read().splitlines()
        images = [image for image in images2 if "JP" not in image]

    if config['organs'] == 'LH' and config['jsrt']:
        images2 = open("train_images_heart.txt",'r').read().splitlines()
        images = [image for image in images2 if "JP" in image]
