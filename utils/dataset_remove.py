...
class LandmarksDataset(Dataset):
    def __init__(self, images, img_path, label_path, transform=None, organ = False, remove = 1):
...
class AugColor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': image, 'landmarks': landmarks, 'name': name}
...
class ToTensorSeg(object):
    ...
    def __call__(self, sample):
        ...
        if self.exp == 1 and 'JPC' in name:
            seg[seg == 1] = 0
        elif self.exp == 2 and 'JPC' in name:
            seg[seg == 2] = 0
        elif self.exp == 3 and 'JPC' not in name:
            seg[seg == 1] = 0
        elif self.exp == 4 and 'JPC' not in name:
            seg[seg == 2] = 0
